const { supabase, cors, safeErr, num, str, today, fetchAll, requireSession, roleAllowed } = require('./_lib/db');
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (action === 'invoice' && req.method === 'GET') {
      ...
      const { data: tenant } = await supabase.from('tenants').select('shop_name,mobile,address,vat_percent,receipt_width_mm,shop_logo').eq('id', ses.tenantId).maybeSingle();
    ...
    if (action === 'today-sales' && req.method === 'GET') {
      const data = await fetchAll(() => supabase.from('transactions').select('*').eq('tenant_id', ses.tenantId).eq('type', 'sale').eq('date', today()));
