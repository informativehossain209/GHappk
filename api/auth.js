...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (action === 'login' && req.method === 'POST') {
      ...
      return res.json({
        ok: true, token,
        tenant: { id: tenant.id, shopName: tenant.shop_name, category: tenant.category, uiMode: tenant.ui_mode, vatPercent: tenant.vat_percent, receiptWidthMm: tenant.receipt_width_mm, lat: tenant.lat, lng: tenant.lng, geoRadiusM: tenant.geo_radius_m },
    ...
    if (action === 'me' && req.method === 'GET') {
      ...
      return res.json({
        ok: true,
        tenant: { id: ses.tenant.id, shopName: ses.tenant.shop_name, category: ses.tenant.category, uiMode: ses.tenant.ui_mode, vatPercent: ses.tenant.vat_percent, receiptWidthMm: ses.tenant.receipt_width_mm, lat: ses.tenant.lat, lng: ses.tenant.lng, geoRadiusM: ses.tenant.geo_radius_m },
