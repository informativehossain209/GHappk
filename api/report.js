...
function periodBounds(type, key, customFrom, customTo) {
  if (type === 'custom') {
    // v4 — free date-range search, not limited to daily/weekly/monthly/yearly.
    const [kFrom, kTo] = String(key || '').split('_');
    const from = customFrom || kFrom;
    const to = customTo || kTo || from;
    return { from, to };
  }
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (action === 'cron-daily' && req.method === 'GET') {
      ...
      const tenants = await fetchAll(() => supabase.from('tenants').select('id').eq('status', 'active'));
    ...
    if (action === 'generate' && req.method === 'GET') {
      const periodType = str(req.query.periodType) || 'daily'; // daily|weekly|monthly|yearly|custom
      const customFrom = str(req.query.from || '');
      const customTo = str(req.query.to || '');
      const periodKey = periodType === 'custom'
        ? (customFrom && customTo ? `${customFrom}_${customTo}` : str(req.query.periodKey))
        : (str(req.query.periodKey) || today());
      const reportType = str(req.query.type) || 'sales';
      if (periodType === 'custom' && (!customFrom || !customTo))
        return res.json({ ok: false, error: 'শুরু ও শেষ তারিখ দিন' });
      const { from, to } = periodBounds(periodType, periodKey, customFrom, customTo);
