...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (action === 'tenant-set-location' && req.method === 'PUT') {
      ...
      const { lat, lng, radiusM, ontimeHour } = req.body || {};
      ...
      if (ontimeHour != null && ontimeHour !== '') patch.punch_ontime_hour = Math.max(0, Math.min(23, num(ontimeHour)));
      const { error } = await supabase.from('tenants').update(patch).eq('id', ses.tenantId);
      if (error) throw error;
      return res.json({ ok: true, ...patch });
    }

    // Owner reads back the shop's current location/geofence/on-time hour —
    // used to prefill the "শপ লোকেশন সেট করুন" card so re-opening it doesn't
    // look blank when a location is already configured.
    if (action === 'tenant-get-location' && req.method === 'GET') {
      if (!roleAllowed(ses.role, ['owner']))
        return res.status(403).json({ ok: false, error: 'অনুমতি নেই' });
      const { lat, lng, geo_radius_m, punch_ontime_hour } = ses.tenant;
      return res.json({ ok: true, lat, lng, radiusM: geo_radius_m, ontimeHour: punch_ontime_hour != null ? punch_ontime_hour : 10 });
    }
    ...
    if (action === 'salary-compute' && req.method === 'GET') {
      ...
      for (const d of (days || [])) {
        ...
        if (d.punch_in && d.punch_out) {
          ...
          const onTimeByHour = ses.tenant.punch_ontime_hour != null ? num(ses.tenant.punch_ontime_hour) : 10; // owner-configurable shop opening/punch time (§ shop location settings)
          if (hour <= onTimeByHour) onTimeDays += 1;
