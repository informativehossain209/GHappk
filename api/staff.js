const { supabase, cors, safeErr, fetchAll, num, str, today, validPin, requireSession, roleAllowed } = require('./_lib/db');
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (action === 'punch' && req.method === 'POST') {
      const { staffId, type } = req.body || {}; // type: 'in' | 'out'
      ...
      if (!existing) {
        ...
        const patch = type === 'in' ? { punch_in: now } : { punch_out: now };
