...
### Version beta v1 (V58 — Premium UI reskin, on top of V57)

A mobile-first stock & sales management web app built with **Vercel serverless functions** + **Supabase** (PostgreSQL).

---

## 🆕 What's New in V58 — Premium matte black + gold UI reskin

This is a **pure visual reskin** on top of V57 — no API changes, no schema
changes, no calculation/logic changes. Every screen, every menu item, every
role's flow works exactly as it did before; only the color system changed.

- **Old theme** was Google-Material blue (`#0D47A1` / `#1565C0` / light-blue
  page background).
- **New theme** is matte black + antique/brushed gold, in both light mode
  (warm ivory background, gold text-accent, near-black header) and dark mode
  (true matte-black background, same gold accent).
- The whole app was already built on a small set of CSS custom properties
  (`--bl`, `--bg`, `--card`, `--bdr`, `--hdr1`/`--hdr2`, etc.) defined once in
  `:root` / `body.dark` in `public/index.html`, so the reskin is contained to
  that variable block plus a handful of hardcoded hex values that had
  bypassed the variables (splash screen, pin screen, showcase/section header
  gradients, role-color map, a few inline `style=` chips). All of those were
  found and swapped too — see the color table below.
- Added a real favicon and iOS "Add to Home Screen" icon using your AXIION
  logo (`public/favicon-32.png`, `public/apple-touch-icon.png`); the full
  logo is also included at `public/logo.png` for future use (e.g. a login
  screen or side-menu header image).
- Status colors (green = success, red = danger, amber = warning, teal,
  purple) were **left untouched** — only the brand/primary color and
  neutral background/border/text tones changed, so due/paid/pending
  indicators etc. still read exactly the same at a glance.

| Token | Was (V57, blue) | Now (V58, gold/black) | Used for |
|---|---|---|---|
| `--bl` / `--bl2` | `#0D47A1` / `#1565C0` | `#8A6A16` / `#B68A22` | headings, links, active nav/menu item, primary accent text |
| `--bll` | `#E3F2FD` | `#F6EFDC` | light tint behind the active menu item / chips |
| `--bg` | `#EEF2FF` | `#F7F3EA` | page background (light mode) |
| `--bg` (dark) | `#0F1923` | `#0B0A08` | page background (dark mode) |
| `--hdr1` / `--hdr2` | `#0D2137` / `#0D47A1` | `#0A0A0C` / `#1C1712` | top header bar + slide-out menu header gradient |
| splash / PIN-screen / punch-card gradients | navy → blue | matte black → bronze → gold sheen | full-screen gradients |
| `#FFD700` (old "Powered by AXIION" gold) | pure gold | `#D4AF37` (refined muted gold) | footer credit, PIN dots, role badge |

**If you want to tweak the palette further**, everything lives in two small
blocks near the top of `public/index.html` — search for `:root{` (light
mode) and `body.dark{` (dark mode) — plus the five gradient lines listed
above (search for `#0A0A0C` to find all of them). Nothing else needs to
change; the rest of the app reads its colors from these variables.

**Next steps once you've reviewed this on a device:** send over the bug
list / calculation fixes / algorithm changes you mentioned and we'll apply
those on top of this same file — the reskin doesn't block or complicate
that work at all.

---

## 🚀 Deploy this (GitHub + Vercel + Supabase)

You already have a live Supabase project from the original V1–V57 rollout,
so this is just shipping new code on top of the same database — **no new
Supabase setup and no SQL migration needed for the V58 reskin itself**
(only run a migration `.sql` file if one is listed for a version you're
jumping to — see `AXIION_V57_partial_migration.sql`).

1. **Push this folder to GitHub**
   ```bash
   cd axiion-dms          # this folder
   git init
   git add .
   git commit -m "V58: premium black/gold UI reskin"
   git branch -M main
   git remote add origin https://github.com/<your-org>/<your-repo>.git
   git push -u origin main
   ```
   If you're updating an *existing* repo instead of starting fresh, just
   copy `public/index.html`, `public/favicon-32.png`,
   `public/apple-touch-icon.png`, and `public/logo.png` into your existing
   repo, commit, and push — the `api/*.js` files are unchanged from V57 so
   you can leave those alone (or overwrite with the copies here, they're
   identical).

2. **Vercel**
   - If the project is already connected to Vercel, pushing to `main`
     auto-deploys — nothing else to do.
   - If this is a fresh Vercel project: **New Project → Import** this
     GitHub repo → Framework preset **Other** → Build command
     `npm install` → Output directory `public` (already set in
     `vercel.json`, Vercel will detect it automatically).
   - Set these **Environment Variables** in Vercel → Project → Settings →
     Environment Variables (use your existing Supabase project's values —
     find them in Supabase → Project Settings → API):
     | Variable | Value |
     |---|---|
     | `SUPABASE_URL` | your Supabase project URL |
     | `SUPABASE_SERVICE_KEY` | Supabase **service_role** key (secret — server-side only) |
     | `SUPABASE_ANON_KEY` | Supabase **anon/public** key |
     | `CRON_SECRET` | any random string, used to protect the daily report cron endpoint |
   - Redeploy after adding/changing env vars (Vercel → Deployments → ⋯ →
     Redeploy).

3. **Supabase** — no action needed for this reskin. Your existing tables,
   RLS policies, and data are untouched. Only re-run
   `AXIION_V57_partial_migration.sql` (or `schema.sql` for a brand-new
   database) if you're setting this up somewhere it hasn't run before.

4. **Verify** — open the deployed URL, confirm the PIN login screen shows
   the new matte-black/gold look, log in, and click through a few sections
   to confirm data still loads (this only touched CSS/colors, so nothing
   functional should have changed).
