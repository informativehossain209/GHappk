...
-- Schema v4 — this is now the ONE complete file: every earlier "partial"
-- migration (v3's location/punch columns, etc.) has been folded in here.
-- There is no separate migration file to run — for a brand-new deploy,
-- run this single script once against a fresh Supabase Postgres project
-- and you're fully up to date. Enable the `pgcrypto` extension first (for
-- gen_random_uuid()) — Supabase projects normally have this on already.
--
-- Migrating an EXISTING v1/v2/v3 database instead of deploying fresh?
-- Every `create table if not exists` / `add column if not exists` here is
-- safe to re-run against live data. The one exception is the new
-- `uniq_products_tenant_name` index a few sections down — if duplicate
-- product names already exist in your data (the exact bug this version
-- fixes), delete/rename the duplicates first or that one statement will
-- fail; everything else in this file will still apply cleanly.
-- ============================================================================
...
create table if not exists tenants (
  ...
  lat                numeric,                    -- shop GPS location — punch-in geofence
  lng                numeric,
  geo_radius_m       int not null default 150,   -- allowed punch distance from lat/lng
  punch_ontime_hour  int not null default 10,    -- owner-set "punch in by" hour (0-23) for on-time bonus
  status             text not null default 'pending'
                       check (status in ('pending','active','suspended','rejected')),
  shop_logo          text default '',            -- v4: owner-uploaded shop picture/logo (settings screen)
...
-- tenants table, so this explicit column-add is what actually delivers
-- shop_logo (and every other v2/v3/v4 tenants column) onto an older DB.
alter table tenants add column if not exists shop_logo text default '';
...
-- Seed a default platform admin so the login has something to authenticate
-- against on a fresh deploy. ON CONFLICT DO NOTHING means this is safe to
-- re-run — it will never overwrite the password after you've changed it.
-- Login: username 'ax203695' / password '209Suncore' — change this any
-- time by generating a new hash and running an UPDATE (ask for help
-- generating the hash whenever you want to rotate it).
insert into platform_admins (name, username, password_hash)
values ('Owner', 'ax203695', 'bdd29516e790f7f9a09733795e53c8b6:08789d083f009508f4c223c33d2555e41ba28afb22284c170bf9a216d75568495a85be1963dcb8d7de1a4cf656c605f0ab0f654a331b6c536505fa55042bd61a')
on conflict (username) do nothing;

-- Brute-force lockout tracking for the platform-admin login (§2.1). Not
-- tenant-scoped, lives outside RLS like platform_admins itself.
...
-- v4: belt-and-suspenders against the "same product saved 2-3-4 times"
-- bug — api/products.js now checks for an existing name before insert,
-- and this unique index is the DB-level backstop in case two requests
-- ever land at the exact same instant (a race the app-level check alone
-- can't fully close).
create unique index if not exists uniq_products_tenant_name on products (tenant_id, lower(name));
...
create table if not exists attendance (
  ...
  in_lat       numeric, in_lng numeric,   -- GPS captured at punch-in
  out_lat      numeric, out_lng numeric,  -- GPS captured at punch-out
...
-- Same no-op-on-existing-table reasoning as tenants.shop_logo above.
...
create table if not exists report_snapshots (
  ...
  period_key   text not null,     -- 'YYYY-MM-DD' | 'YYYY-Www' | 'YYYY-MM' | 'YYYY' | 'YYYY-MM-DD_YYYY-MM-DD' (custom range)
...
-- ============================================================================
-- 10. STORAGE BUCKET (§3) — product/staff photo uploads land here via
--     api/_lib/thumbStorage.js. This used to require a manual "create a
--     bucket named thumbs" step in the Supabase dashboard after running
--     this file — easy to forget, and forgetting it is exactly what caused
--     the "bucket not found" error on product creation. Creating it here
--     instead means running this single script is enough, nothing else
--     to remember. storage.buckets is a normal table Supabase exposes, so
--     this insert works from the SQL editor same as any other statement.
-- ============================================================================
insert into storage.buckets (id, name, public)
values ('thumbs', 'thumbs', true)
on conflict (id) do update set public = true;

-- Public bucket downloads (the getPublicUrl() links stored on products.thumb
-- /staff.thumb) are served by Storage's public endpoint and don't consult
-- these policies — but upload/list/delete from anything OTHER than the
-- service-role key (which api/_lib/thumbStorage.js uses, and which bypasses
-- RLS entirely) would need them, so they're included as the same
-- defense-in-depth as the table RLS policies below.
